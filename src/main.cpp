      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
        	...
        	// Main car's localization Data
					...
					int points_ahead = 30;
					vector<double> next_wp0 = getXY((car_s + 1*points_ahead), 2 + (4*lane), map_waypoints_s, map_waypoints_x, map_waypoints_y);
					vector<double> next_wp1 = getXY((car_s + 2*points_ahead), 2 + (4*lane), map_waypoints_s, map_waypoints_x, map_waypoints_y);
					vector<double> next_wp2 = getXY((car_s + 3*points_ahead), 2 + (4*lane), map_waypoints_s, map_waypoints_x, map_waypoints_y);
					...
					double target_x = 30.0; //horizon - 30 meters
					...
					for(int i=1; i<= 50-previous_path_x.size(); i++){
						double N = (target_dist / (.02 * ref_vel / 2.24)); //2.24 is meters/sec
					...
					// 	double next_d = 4*1.5; //4 meters * 1.5 lanes
